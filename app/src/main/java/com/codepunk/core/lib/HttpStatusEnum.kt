...
enum class HttpStatusEnum(
    ...
    companion object {
        ...
        /**
         * A lookup [SparseArray] for looking up [HttpStatusEnum] values by status code.
         */
        private val lookupArray by lazy {
            SparseArray<HttpStatusEnum>(values().size).apply {
                for (type in HttpStatusEnum.values()) {
                    put(type.code, type)
                }
            }
        }
        ...
        /**
         * Gets the [HttpStatusEnum] associated with the supplied [code], or null if no such value
         * exists.
         */
        @Suppress("WEAKER_ACCESS")
        fun lookup(code: Int): HttpStatusEnum? = lookupArray[code]

        /**
         * Gets a reasonPhrase this http status.
         */
        fun descriptionOf(code: Int?, notFoundText: String = "Unknown"): String? =
            when (code) {
                null -> null
                else -> lookup(code)?.description ?: "$code $notFoundText"
            }
